...
+ Works with eshell
+ Bonus: Significantly faster than ansi-color.el
...
For programmatic usage, in Emacs Lisp code call ~xterm-color-filter~ to
propertize strings that you can then insert into a buffer. All state is kept in
